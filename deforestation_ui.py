...
def main():
    ...
    with main_container:
        ...
        with st.expander("Advanced Settings"):
            ...
            target_lat = st.number_input("Latitude", value=0.0, format="%.6f", help="Target latitude for NGO matchmaking")
            target_lon = st.number_input("Longitude", value=0.0, format="%.6f", help="Target longitude for NGO matchmaking")
            ...
            with st.spinner("Processing images... Please wait"):
                try:
                    ...
                    if analysis_mode == "Deforestation Analysis":
                        ...
                        st.markdown('</div>', unsafe_allow_html=True)
                        damaged_area_m2 = deforested_area
                    ...
                    # Forest fire detection
                        ...
                        try:
                            ...
                            st.markdown('</div>', unsafe_allow_html=True)
                            damaged_area_m2 = burn_area * pixel_resolution**2
                        except Exception as e:
                    ...
                    # Structured handoff point for later phases
                    if 'damaged_area_m2' in locals() and damaged_area_m2 > 0:
                        region_count = num_regions if analysis_mode == "Deforestation Analysis" else num_burn_regions
                        
                        st.session_state.latest_analysis = {
                            "mode": analysis_mode,
                            "damage_area_m2": damaged_area_m2,
                        ...
                        st.markdown('<div class="card" style="margin-top: 30px; border-left: 5px solid #2e7d32; background-color: rgba(255, 255, 255, 0.95);">', unsafe_allow_html=True)
                        st.markdown('<h2 style="color: #0a3412; font-family: Georgia, serif; text-align: center;">Active Recovery Action Plan</h2>', unsafe_allow_html=True)
                        
                        trees, cost, summary = calculate_reforestation_economics(damaged_area_m2)
                        
                        # Best-effort DB history logging
                        db.log_analysis(analysis_mode, target_lat, target_lon, damaged_area_m2, cost)
                        
                        rec_cols = st.columns(2)
                        with rec_cols[0]:
                            st.metric("Required Saplings (1 per 4m²)", f"{trees:,.0f}")
                        with rec_cols[1]:
                            st.metric("Estimated Cost ($2.50/tree)", f"${cost:,.2f}")
                            
                        st.markdown('<h3 style="color: #0a3412; font-family: Georgia, serif; margin-top: 20px;">Local NGO Partners for Reforestation</h3>', unsafe_allow_html=True)
                        with st.spinner("Finding nearby organizations..."):
                            ngos = find_nearby_ngos(target_lat, target_lon)
                            
                            if ngos:
                                is_fallback = any(ngo.get('source') == 'Fallback Database' for ngo in ngos)
                                if is_fallback:
                                    st.caption("Showing regional partner organizations")
                                    
                                for ngo in ngos:
                                    st.markdown(f"**{ngo['name']}** ({ngo['category']}) - *Location: {ngo['location']}*")
                                    st.markdown(f"Contact: {ngo['contact']} | [Website]({ngo['website']})")
                                    st.markdown("---")
                            else:
                                st.info("No NGOs found in your area.")
                        st.markdown('</div>', unsafe_allow_html=True)
